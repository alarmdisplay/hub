...
export class Locations extends Service<LocationData> {
  ...
  async processRawLocation(rawLocation: RawLocation): Promise<LocationData> {
    ...
    if (rawLocation.gk && !(location.latitude && location.longitude)) {
      try {
        const gkCoordinates = { x: Number(rawLocation.gk.x), y: Number(rawLocation.gk.y) };
        const wgsCoordinates = gk.toWGS(gkCoordinates);
  ...
  convertToDecimalNumber(degrees: string, minutes: string, seconds: string): number|undefined {
    ...
    const minutesFloat = Number.parseFloat(minutes);
    ...
    const secondsFloat = Number.parseFloat(seconds);
